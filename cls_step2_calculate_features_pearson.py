...
def calculate_feature_for_a_ratio(
    edge_ratio_str: str,
    input_dir: str | Path,
    ...
    for fname in files_for_ratio:
        ...
        for i_net, network_1 in enumerate(network_names):
            ...
            for j_net, network_2 in enumerate(network_names):
                ...
                # erived features: max_div_std & max_x_std
...
if __name__ == '__main__':
    ...
    expected_edge_ratios = [i * 0.02 for i in range(1, 50)]
    ratio_strings = [f'{ratio:.02f}' for ratio in expected_edge_ratios]
    
    # --- Path Setup ---
    base_in_folder = Path('graphs-pearson') / 'FunImgARglobalCWSF' / 'bna'
    
    base_out_folder = Path('graphs-pearson') / 'FunImgARglobalCWSF' / 'bna-feat'
    ...
    all_filenames = [f for f in os.listdir(base_in_folder) if f.endswith('.npy')]
    ...
    for ratio_str in ratio_strings:
        ...
        list_for_jobs.append(
            delayed(calculate_feature_for_a_ratio)(
                edge_ratio_str=ratio_str,
                input_dir=base_in_folder,
    ...
    print(f"All feature extraction completed.")
