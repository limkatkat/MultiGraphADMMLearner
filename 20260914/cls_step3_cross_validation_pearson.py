...
if __name__ == '__main__':
    ...
    for yeo_nets, test_params in zip(YEO_NETWORK_COUNTS, test_params_list):
        ...
        for feature_type in feature_types:
            ...
            result_csv_path = output_dir / f'{group_name}-model_eval_results{suffix}-Yeo{yeo_nets}-{feature_id}-{test_params_str}.csv'
            ...
            for edge_ratio in edge_ratios:
                print("\n" + "="*60)
                print(f" Loading Data: Edge Ratio = {edge_ratio}, Feature Type = {feature_type}")
                print("="*60)
                
                # Load Data
                feat_file_path = data_dir / f'Yeo{yeo_nets}-{edge_ratio:.02f}.csv'
                ...
                for current_clf in CLF_MANAGERS:
                    ...
                    np.save(output_dir / f'{prefix}-rocdata.npy', roc_data)
                    np.save(output_dir / f'{prefix}-weights.npy', avg_weights)
                    np.save(output_dir / f'{prefix}-freq.npy', avg_freq)
